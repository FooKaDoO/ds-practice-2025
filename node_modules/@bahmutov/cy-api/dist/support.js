...
Cypress.on('test:before:run', () => {
    ...
    globalDisplayRequest =
        apiDisplayRequest === undefined ? true : apiDisplayRequest;
...
Cypress.Commands.add('api', (options, name = 'api') => {
    ...
    const messagesEndpoint = get(Cypress.env(), 'cyApi.messages', '/__messages__');
    ...
    if (apiOptions.displayRequest) {
        if (firstApiRequest) {
          ...
          <link
            rel="stylesheet"
            href="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/${pack['dependencies']['highlight.js']}/styles/vs.min.css"
          />
          <style>
            .container {
...
const printResponse = (container, hasApiMessages, messagesEndpoint, normalizedTypes, normalizedNamespaces, displayRequest = true) => {
    let messages = [];
    if (hasApiMessages) {
        return cy
            .request({
                ...
                if (displayRequest) {
                    ...
                    container.innerHTML +=
                        '\n<pre class="cy-api-logs-messages">' +
                            messages
                                .map((m) => {
                                const s = typeof m.message === 'string'
                                    ? m.message
                                    : JSON.stringify(m.message);
                                const html = `<div class="${normalize(m.type)} ${normalize(m.type, m.namespace)}">${m.type} ${m.namespace}: ${s}</div>`;
                                return html;
                            })
                                .join('') +
                            '\n</pre></div>';
...
const addOnClickFilter = (filterId) => {
    // @ts-ignore
    const doc = cy.state('document');
...
const getContainer = () => {
    ...
    if (!container) {
        ...
        // in Cypress v12
        const innerContainer = doc.querySelector('.inner-container');
        if (innerContainer) {
